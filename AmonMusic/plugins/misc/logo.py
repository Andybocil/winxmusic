...
from config import BOT_USERNAME, OWNER_ID, MUSIC_BOT_NAME, SUPPORT_GROUP
...
async def lego(client, message):
    if message.from_user.id != OWNER_ID:
        # Check if the message has enough parts after the command
    ...
    await message.reply("**ᴄʀᴇᴀᴛɪɴɢ ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛᴇᴅ ʟᴏɢᴏ ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ ᴀ sᴇᴄ​...**")
    try:
...
━━━━━━━{MUSIC_BOT_NAME}━━━━━━━""",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("๏ sᴜᴘᴘᴏʀᴛ ๏", url=SUPPORT_GROUP)]])
        )
        ...
        await client.send_message(
            message.chat.id,
            f"ᴇʀʀᴏʀ {e}, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ {SUPPORT_GROUP}"
        )
